...
t-deploy-lab() { 
	...
	if [[ $(awk -F= -v key="msr_version_3" '$1==key {print $2}' /terraTrain/config  | tr -d '"' | cut -d' ' -f1 | tr -d "\n") == "1" ]] 
		then 
			(launchpad apply --config launchpad.yaml &> /tmp/mke-installation.log ; if [ $? -eq 0 ] ;	then /bin/t install msrv3 &>> /tmp/mke-installation.log  ; else echo "MSR Installation Failed" ; fi ) &
	else
